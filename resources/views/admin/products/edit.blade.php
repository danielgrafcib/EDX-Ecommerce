    ...
    <section class="space-y-6">
        ...
        <form method="post" action="{{ route('admin.products.update', $product->id) }}" class="space-y-6" enctype="multipart/form-data">
                ...
                <label class="text-xs font-medium text-neutral-300">
                    ...
                    <select name="market_id" class="mt-1 w-full rounded-lg border border-neutral-700 bg-neutral-950 px-3 py-2 text-sm text-neutral-50">
                        ...
                        @foreach($markets as $m)
                            <option value="{{ $m->id }}" @selected(old('market_id', $product->market_id) == $m->id)>{{ $m->name }}</option>
                        @endforeach
                    </select>
                </label>
                <label class="text-xs font-medium text-neutral-300">
                    ...
                    <select name="enterprise_id" class="mt-1 w-full rounded-lg border border-neutral-700 bg-neutral-950 px-3 py-2 text-sm text-neutral-50">
                        ...
                        @foreach($enterprises as $e)
                            <option value="{{ $e->id }}" @selected(old('enterprise_id', $product->enterprise_id) == $e->id)>{{ $e->name }}</option>
                        @endforeach
                    </select>
                </label>
                <label class="text-xs font-medium text-neutral-300">
            ...
            <div class="grid gap-4 md:grid-cols-3">
                <label class="text-xs font-medium text-neutral-300">
                    Prix promotionnel (€)
                    <input name="price_promo" type="number" step="0.01" value="{{ old('price_promo', $product->price_promo) }}" class="mt-1 w-full rounded-lg border border-neutral-700 bg-neutral-950 px-3 py-2 text-sm text-neutral-50">
                </label>
                <label class="text-xs font-medium text-neutral-300">
                    Prix partenaire (€)
                    <input name="price_partner" type="number" step="0.01" value="{{ old('price_partner', $product->price_partner) }}" class="mt-1 w-full rounded-lg border border-neutral-700 bg-neutral-950 px-3 py-2 text-sm text-neutral-50">
                </label>
                <label class="text-xs font-medium text-neutral-300">
                    Prix premium (€)
                    <input name="price_premium" type="number" step="0.01" value="{{ old('price_premium', $product->price_premium) }}" class="mt-1 w-full rounded-lg border border-neutral-700 bg-neutral-950 px-3 py-2 text-sm text-neutral-50">
                </label>
            </div>
